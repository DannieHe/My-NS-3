...
#define SIM_START	01.00
#define SIM_STOP	11.00
...
main (int argc, char *argv[])
{
  ...
  //app1.Start(Seconds(SIM_START + 0.1));
  //app1.Stop(Seconds(SIM_STOP - 0.1));
  // 送信設定
  int cnt = 0;
  for(size_t i = 0; i < client_nodes.size(); i++){
	  if(cnt < 2){
		  UdpClientHelper client(hostIpIfaces.GetAddress(10), PORT);
		  client.SetAttribute("MaxPackets", UintegerValue((SIM_STOP - SIM_START)*(packet_rate/4)));
  ...
  app1.Start(Seconds(SIM_START));
  app1.Stop(Seconds(SIM_STOP));
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      ...
      std::cout << "  TxOffered:  " << i->second.txBytes * 8.0 / (SIM_STOP - SIM_START) / 1000 / 1000  << " Mbps\n";
      std::cout << "  Rx Packets: " << i->second.rxPackets << "\n";
      std::cout << "  Rx Bytes:   " << i->second.rxBytes << "\n";
      std::cout << "  Throughput: " << i->second.rxBytes * 8.0 / (SIM_STOP - SIM_START) / 1000 / 1000  << " Mbps\n";
    }
